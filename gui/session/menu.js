...
const RESOURCES = ["food", "wood", "stone", "metal","water"];
const STEP = 5;
...
function openDiplomacy()
{
	...
	for (var i = 1; i < players.length; i++)
	{
		...
		if (i == we || players[we].state != "active" || players[i].state != "active")
		{
			...
			for each (var a in ["TributeFood", "TributeWood", "TributeStone", "TributeMetal","TributeWater", "Ally", "Neutral", "Enemy"])
				Engine.GetGUIObjectByName("diplomacyPlayer"+a+"["+(i-1)+"]").hidden = true;
			continue;
		}

		// Tribute
		for each (var resource in ["food", "wood", "stone", "metal","water"])
		{
			...
			button.onpress = (function(player, resource, button){
				...
				return function() {
					...
					var amounts = {
						...
						"water": (resource == "water" ? 100 : 0) * multiplier,
					};

...
function getEntityCostComponentsTooltipString(template, trainNum, entity)
{
	...
	if (totalCosts.water) costs.push(sprintf(translate("%(component)s %(cost)s"), { component: getCostComponentDisplayName("water"), cost: totalCosts.water }));
...
function getLocalizedResourceName(resourceCode, context)
{
    if (context == "firstWord")
    {
        switch(resourceCode)
        {
			...
			case "water": return translateWithContext("firstWord", "Water");
            // Translation: Word as used at the beginning of a sentence or as a single-word sentence.
    ...
    else if (context == "withinSentence")
    {
        switch(resourceCode)
        {
			...
			case "water": return translateWithContext("withinSentence", "Water");
            // Translation: Word as used in the middle of a sentence (which may require using lowercase for your language).

...
GuiInterface.prototype.GetTemplateData = function(player, extendedName)
{
	...
	if (template.Cost)
	{
		...
		try
		{
		if (template.Cost.Resources.water) 
		ret.cost.water = ApplyValueModificationsToTemplate("Cost/Resources/water", +template.Cost.Resources.water, player, template);
		else
			ret.cost.water = ApplyValueModificationsToTemplate("Cost/Resources/water", +0, player, template);
		}
		catch(err)
		{
...
GuiInterface.prototype.GetTechnologyData = function(player, name)
{
	...
	//this var is to keep errors from the not having water in techs
	var u=0;
	//if u=0 try to add a variable
	if (u==0)
	{
	//this tries to add water
	try{
		ret.cost = {
		"water":template.cost ? (+template.cost.water):0,
	...
	if (u==1)
	{
	//this is the default, keeps it normal if no water
	ret.cost = {
		"food": template.cost ? (+template.cost.food) : 0,
		"wood": template.cost ? (+template.cost.wood) : 0,
		"metal": template.cost ? (+template.cost.metal) : 0,
...
GuiInterface.prototype.SetWallPlacementPreview = function(player, cmd)
{
	...
	var result = {
		"pieces": [],
		"cost": {"food": 0,"water": 0, "wood": 0, "stone": 0, "metal": 0, "population": 0, "populationBonus": 0, "time": 0},
	};
